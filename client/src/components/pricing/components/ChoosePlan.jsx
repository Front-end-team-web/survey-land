...
const ChoosePlan = () => {
    ...
    return (
        <div className={`px-4 mx-auto max-w-7xl`}>
            ...
            >
                <motion.h1 variants={item} className="font-bold text-4xl md:text-6xl leading-tight">
                    <motion.span
                        className="text-[#7616EC]"
                        variants={item}
                    >
                        {t('pages.pricing.ChoosePlan.Choose')}
                    </motion.span> {t('pages.pricing.ChoosePlan.your Plan')}
                </motion.h1>

                <motion.p variants={item} className="font-normal text-xl md:text-2xl lg:text-3xl leading-snug md:leading-normal">
                    {t('pages.pricing.ChoosePlan.Choose your perfect plan')}
                </motion.p>

                <motion.p variants={item} className="text-lg md:text-xl lg:text-2xl leading-relaxed w-full md:w-4/5 lg:w-3/4 mx-auto text-center">
                    {t('pages.pricing.ChoosePlan.Choose the perfect')}
                </motion.p>

                <motion.div variants={item} className="flex flex-wrap justify-center gap-3 md:gap-5 lg:gap-8 mx-auto w-full max-w-md mt-8">
                    <motion.button
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        className={`border-2 rounded-xl py-2 px-4 md:px-6 lg:px-8 text-gray-600 border-gray-600 font-semibold text-base md:text-lg whitespace-nowrap ${isDarkMode ? 'text-white border-white' : ''}`}
                    >
            ...
            >
                {Plans.map((plan, index) => (
                    <motion.div
                        ...
                        variants={cardItem}
                        whileHover={{ y: -5 }}
                        className={`border-2 border-[--main-color] p-6 md:p-8 rounded-2xl flex flex-col relative h-fit ${isDarkMode && index !== 1 ? 'text-white border-white' : ''} 
                        ${index === 1 ? "bg-[--main-color] text-white min-h-[660px]" : "text-[#490F8F] min-h-[620px]"}`}
                    >
                        {index === 1 && (
                            <motion.div
                        ...
                        <h3 className="text-2xl md:text-3xl font-bold mt-2">{t(`pages.pricing.ChoosePlan.${plan.title}`)}</h3>
                        <p className="mt-4">
                            <span className="text-2xl md:text-3xl font-bold">{plan.price}</span>/
                            {plan.time}
                        </p>
                        <p className="my-4 text-base md:text-lg">{t(`pages.pricing.ChoosePlan.PlanDescriptions.${plan.title}`)}</p>
                        ...
                        <motion.button
                            ...
                            whileTap={{ scale: 0.97 }}
                            className={`mt-auto border-2 rounded-xl py-2 px-6 font-semibold text-lg w-full mx-auto ${index === 1 ? "bg-white text-[#490F8F] border-white" : "bg-[--main-color] text-white border-[--main-color]"}`}
                        >
